/*
  MQ-8 Gas Sensor with Arduino
  ------------------------------------
  This program reads the analog output value (0-1023) from the MQ-8 gas sensor 
  and prints it to the serial monitor.

  Notes:
  - The MQ-8 sensor is highly sensitive to hydrogen (H₂).
  - It can also detect LPG, methane, alcohol, and carbon monoxide at lower sensitivity.
  - After powering on, the sensor requires a warm-up time of at least 1-2 minutes.
  - The sensor's analog output varies with hydrogen concentration in the air.

  Sensor:      MQ-8 Gas Sensor
  Connection:  VCC -> 5V, GND -> GND, A0 -> Analog Pin A0
*/

// Define the analog pin connected to the MQ-8 gas sensor
...
void setup() {
  ...
  Serial.println("MQ-8 Gas Sensor Initialization...");
...
void loop() {
  ...
  if (sensorValue > threshold) {
    Serial.println("Warning! High Hydrogen Level Detected!");
  }
